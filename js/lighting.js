...
function applyLighting(flickers) {
  ...
  // Torches — tinted by each torch's color, scaled by per-torch opacity
  TORCHES.forEach(([tr,tc,color,opac],i)=>{
    const op = (opac !== undefined ? opac : 1.0);
    ...
    g.addColorStop(0,  `rgba(${br},${bg},${bb},${(1.0 *op).toFixed(2)})`);
    g.addColorStop(.35,`rgba(${cr},${cg},${cb},${(0.82*op).toFixed(2)})`);
    g.addColorStop(.65,`rgba(${cr},${cg},${cb},${(0.45*op).toFixed(2)})`);
  ...
  TORCHES.forEach(([tr,tc,color,opac],i)=>{
    const op = (opac !== undefined ? opac : 1.0);
    const tx=tc*TW+TW*.5-camX, ty=tr*TH+TH*.5-camY, radius=TS*(2.4+flickers[i]*.35);
    ...
    g.addColorStop(0,  `rgba(${cr},${cg},${cb},${(0.32*op).toFixed(2)})`);
    g.addColorStop(.5, `rgba(${cr},${cg},${cb},${(0.14*op).toFixed(2)})`);
    g.addColorStop(1,  'rgba(0,0,0,0)');
